...
import com.team04.spring.premium.dao.PremiumDao;
...
public class PremiumServiceImpl implements PremiumService{
	...
	public void getList(ModelAndView mView, HttpServletRequest request) {
		...
		*/
		...
		//startRowNum 과 endRowNum  을 PremiumDto 객체에 담고
		PremiumDto dto=new PremiumDto();
		dto.setStartRowNum(startRowNum);
		dto.setEndRowNum(endRowNum);
		
		//ArrayList 객체의 참조값을 담을 지역변수를 미리 만든다.
		List<PremiumDto> list=null;
		...
		//글목록 얻어오기
		list=premiumDao.getlist(dto);
		//글의 갯수
		totalRow=premiumDao.getCount(dto);
	...
	public void getDetail(int num, ModelAndView mView) {
		PremiumDto dto=premiumDao.getData(num);
		mView.addObject("dto",dto);
		//조회수
		premiumDao.addViewCount(num);
	...
	public void updateContent(PremiumDto dto) {
		premiumDao.update(dto);
		
	}
	@Override
	public void deleteContent(int num) {
		premiumDao.delete(num);
